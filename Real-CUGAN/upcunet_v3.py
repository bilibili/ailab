...
def q(inp,cache_mode):
    ...
    if(cache_mode==2):
        ...
        return ((inp-minn)/delta*255).round().byte(),delta,minn,inp.device#不用CPU转移#大概3倍延时#太慢了，屏蔽该模式
...
class UpCunet2x(nn.Module):#完美tile，全程无损
    ...
    def forward(self, x,tile_mode,cache_mode):#1.7G
        n, c, h0, w0 = x.shape
        ...
        if(tile_mode==0):#不tile
            ...
            tile_mode=min(5,int(tile_mode))
            t2=tile_mode*2
            crop_size=(((h0-1)//t2*t2+t2)//tile_mode,((w0-1)//t2*t2+t2)//tile_mode)#3.7G
        else:
        ...
        return res#
    def forward_gap_sync(self, x,tile_mode):#1.7G
        n, c, h0, w0 = x.shape
        if("Half" in x.type()):if_half=True
        else:if_half=False
        if(tile_mode==0):#不tile
...
class UpCunet4x(nn.Module):#完美tile，全程无损
    ...
    def forward(self, x,tile_mode,cache_mode):
        n, c, h0, w0 = x.shape
        if("Half" in x.type()):if_half=True
        else:if_half=False
        x00 = x
...
class RealWaifuUpScaler(object):
    ...
    def __call__(self, frame,tile_mode,cache_mode):
        with torch.no_grad():
            tensor = self.np2tensor(frame)
            if(cache_mode==2):
                result = self.tensor2np(self.model.forward_gap_sync(tensor,tile_mode))
            else:
                result = self.tensor2np(self.model(tensor,tile_mode,cache_mode))
        return result
        ...
        for tile_mode in [0,5]:
            for cache_mode in [0,1,2]:
                ...
                for name in os.listdir(input_dir):
                    ...
                    t0 = ttime()
                    result = upscaler2x(frame, tile_mode=tile_mode,cache_mode=cache_mode)[:, :, ::-1]
                    t1 = ttime()
